...
When the user requests spawning a worktree with an agent in a background session ("spawn a worktree for...", "hand off to another agent"), use the appropriate pattern for their terminal multiplexer. Substitute `<agent-cli>` with the CLI you are running as: `agy` for Antigravity.
...
Example (tmux, Antigravity):
```bash
tmux new-session -d -s fix-auth-bug "wt switch --create fix-auth-bug -x agy -- \
  'The login session expires after 5 minutes. Find the session timeout config and extend it to 24 hours.'"
```

### Parallel sub-Agents (single Antigravity session)

To spawn multiple sub-Agents that each work in their own worktree from one Antigravity session, use the `invoke_subagent` tool with `Workspace: "branch"` or `Workspace: "share"`.

You do not need to use `wt switch --create` because Antigravity natively supports creating branched or shared workspaces for subagents using the `Workspace` parameter in `invoke_subagent`.
...
[
  {
    "TypeName": "research",
    "Role": "Auth Debugger",
    "Prompt": "Find the session timeout config and extend it to 24 hours.",
    "Workspace": "share"
  }
]
```

This will automatically create a new workspace sharing the parent's underlying repository directory (similar to `wt switch --create` or `git worktree add`), and the subagent will be able to work in that workspace independently.
