...
4. **Create Linear Ticket**:
   ...
   {
     "title": "<Title>",
     "description": "<contents of the temporary description file>",
     "team": "4670d896-c578-43a1-b8cf-50043d74d669",
     "project": "c8f35a99-2b16-41a9-ae0f-48e32dbd4237",
     "state": "783efcac-f56e-4732-ba56-45f68ded7961",
     "assignee": "fad7aa89-be83-4fd3-bce8-7e9d577a16e1",
     "milestone": "Omni-channel Communication (Pedro - M, Simon/Joe - ?)"
   }
   ```
   *Delete the temporary description file afterwards.* Capture the new issue's identifier (e.g., `RHL-1234`) from the tool response for the worktree name. If the MCP tool is unavailable, fall back to the `linear issue create` command using the names from the `## Linear IDs` section:

   ```bash
   linear issue create --title "<Title>" --team "RHL" --project "QA Project" \
     --milestone "Omni-channel Communication (Pedro - M, Simon/Joe - ?)" \
